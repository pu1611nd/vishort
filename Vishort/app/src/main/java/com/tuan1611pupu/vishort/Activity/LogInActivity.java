...
public class LogInActivity extends AppCompatActivity {
                    ...
                    if (task.isSuccessful()) {
                        ...
                        if (documentSnapshot != null && documentSnapshot.exists()) {
                            ...
                            if (user != null && user.getProvider() == null) {
                                ...
                                preferenceManager.putString(Constants.KEY_USER_NAME, user.getUsername());
    ...
    private void SingIn(FirebaseUser user, String provider) {
        ...
        user1.setId(userId);
        user1.setImage(user.getPhotoUrl()+"");
                ...
                .document(userId).set(user1).addOnCompleteListener(new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
                            ...
                            preferenceManager.putString(Constants.KEY_USER_NAME, fullName);
                            preferenceManager.putString(Constants.KEY_EMAIL, email);
                            preferenceManager.putString(Constants.KEY_IMAGE, user.getPhotoUrl()+"");
                            Intent intent = new Intent(getApplicationContext(), MainActivity.class);
                            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
                            startActivity(intent);
                            finish();
    ...
    private void singInGoogle(FirebaseUser user, GoogleSignInAccount account) {
        ...
        user1.setId(userId);
        user1.setImage(account.getPhotoUrl()+"");
